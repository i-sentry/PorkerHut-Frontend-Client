      ...
      <div className="md:flex xxs:px-6 md:4 py-8 md:gap-10 bg-white rounded-sm">
        <div className="flex md:flex-1">
          <div className="flex-[1]">
            <img src={images[0]} alt="img1" onClick={e => setSelectedImg(0)} className="object-cover cursor-pointer w-[75px] h-20 mb-3 rounded-sm" />
            <img src={images[1]} alt="img2 rounded" onClick={e => setSelectedImg(1)} className="object-cover cursor-pointer w-[75px] h-20 mb-3 rounded-sm" />
            <img src={images[2]} alt="img3 rounded" onClick={e => setSelectedImg(2)} className="object-cover cursor-pointer w-[75px] h-20 mb-3 rounded-sm" />
            <img src={images[2]} alt="" onClick={e => setSelectedImg(2)} className="object-cover cursor-pointer w-[75px] h-20 rounded" />
          </div>

          <div className="md:flex-[5]">
            <img src={images[selectedImg]} alt="img4" className=" object-cover h-[400px] w-full rounded-sm" />
          </div>
        </div>
        ...
        <span className="font-normal text-sm text-[#797979]">Weight:  <span className="font-medium text-black text-sm">{item?.product?.Weight}</span></span>
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center">
              ...
              <button className="border w-10 h-10" onClick={() => setQuantity(prev => prev === 1 ? 1 : prev - 1)}>-</button>
              <span className="border w-10 h-10 flex items-center justify-center">

                {quantity}
              </span>
              <button className="border w-10 h-10" onClick={() => setQuantity(prev => prev + 1)}>+</button>
            </div>
          </div>
      ...
      <div className="bg-white p-4 rounded-sm">
        ...
        <Fragment>
          <Accordion open={open === 1}>
            <AccordionHeader onClick={() => handleOpen(1)} className="font-medium text-base">
              Product Description
            </AccordionHeader>
            <AccordionBody>
              Our raw bacon is a versatile ingredient that provides a fresh, mild, and savory flavor to any dish. Sourced from premium-quality pigs, our raw bacon is made from the belly of the pig and has not been cured or
              smoked, making it perfect for those who prefer the natural taste of pork. Whether you're making a classic bacon, egg, and cheese sandwich, adding a touch of savory flavor to your favorite pasta dish, or simply
              enjoying a slice of bacon as a snack, our raw bacon is the perfect addition. Keep in mind that raw bacon must be cooked thoroughly before consuming to reduce the risk of foodborne illness. Try it today and
              taste the difference that comes from using high-quality, raw bacon.
            </AccordionBody>
          </Accordion>
