...
import { useGetAllRatingByUser } from "../../../services/hooks/users/products";
...
const CustomerRating = () => {
  ...
  const { data: allUserRatings, isLoading } = useGetAllRatingByUser(
    storedUser?._id
  );
  ...
  const allRatings = allUserRatings?.data?.ratings;
  useEffect(() => setData(allRatings), [allRatings]);

  console.log(storedUser, allUserRatings, "User, All Ratings");
  return (
    <>
      <NavBar />
          ...
          {data &&
            data.map((rating: any, index: number) => (
              ...
              >
                ...
                <div>
                  <span className=" text-sm">{rating?.comment}</span>
                </div>
              </div>
            ))}
        </div>
